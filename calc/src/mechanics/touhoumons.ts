...
export function calculateTH(
  ...
  checkSurprise(gen, attacker, defender);
  checkSurprise(gen, defender, attacker);
  ...
  if (attacker.hasAbility('Vampirism') && move.flags.bite) {
    move.drain = [1, 4];
  }
  ...
  if (attacker.hasAbility('Neutralizing Gas', 'Void') &&
  ...
  if (!move.isZ && !noTypeChange) {
    ...
    if ((isAerilate = attacker.hasAbility('Aerilate') && normal)) {
      ...
      type = 'Normal';
    } else if ((isNightSong = attacker.hasAbility('Night Song') && !!move.flags.sound)) {
      type = 'Dark';
    }
    if (isGalvanize || isPixilate || isRefrigerate || isAerilate || isNormalize) {
      desc.attackerAbility = attacker.ability;
      hasAteAbilityTypeChange = true;
    } else if (isLiquidVoice || isNightSong) {
      desc.attackerAbility = attacker.ability;
    }
  }
  ...
  if ((defender.hasAbility('Wonder Guard') && typeEffectiveness <= 1) ||
      ...
      (move.flags.sound && !move.named('Clangorous Soul') &&
        defender.hasAbility('Soundproof', 'Echo')) ||
      (move.priority > 0 && defender.hasAbility('Queenly Majesty', 'Dazzling', 'Armor Tail')) ||
      (move.hasType('Ground') && defender.hasAbility('Earth Eater')) ||
      (move.flags.wind && defender.hasAbility('Wind Rider')) ||
      (move.hasType('Ghost') && defender.hasAbility('Heal by Desire'))
  ) {
    desc.defenderAbility = defender.ability;
    return result;
  }
  ...
  const basePower = calculateBasePowerTH(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackTH(gen, attacker, defender, move, field, desc, isCritical);
  const attackStat =
  ...
  const defense = calculateDefenseTH(gen, attacker, defender, move, field, desc, isCritical);
  ...
  const baseDamage = calculateBaseDamageTH(
    gen,
    attacker,
    defender,
    basePower,
  ...
  const finalMods = calculateFinalModsTH(
    gen,
    attacker,
    defender,
    move,
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateTH(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (move.timesUsed! > 1 || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAttack = calculateAttackTH(gen, attacker, defender, move,
        field, desc, isCritical);
      const newDefense = calculateDefenseTH(gen, attacker, defender, move,
        field, desc, isCritical);
      ...
      const newBasePower = calculateBasePowerTH(
        gen,
        attacker,
        defender,
        move,
      ...
      const newBaseDamage = calculateBaseDamageTH(
        gen,
        attacker,
        defender,
        newBasePower,
      ...
      const newFinalMods = calculateFinalModsTH(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerTH(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const bpMods = calculateBPModsTH(
    gen,
    attacker,
    defender,
    move,
...
export function calculateBPModsTH(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (field.hasWeather('Snow') && move.hasType('Fire') &&
    attacker.hasAbility('Winter Hearth')) {
    bpMods.push(8192);
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (attacker.hasAbility('Illusory Backing') && move.flags.sound) {
    bpMods.push(5325);
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (attacker.hasAbility('Devil\'s Recitation') && field.hasTerrain('Psychic') &&
    move.hasType('Dark')) {
    bpMods.push(6144);
    desc.attackerAbility = attacker.ability;
  }
...
export function calculateAttackTH(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const atMods = calculateAtModsTH(gen, attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsTH(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if ((attacker.hasAbility('Slow Start') && attacker.abilityOn &&
  ...
  } else if (attacker.hasAbility('Wicked Power') && attacker.abilityOn && move.hasType('Dark')) {
    atMods.push(8192);
    desc.attackerAbility = attacker.ability;
  } else if (
    ...
    atMods.push(8192);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Anemic') && move.category === 'Special') {
    atMods.push(4915);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Dragon Eater') && defender.hasType('Dragon')) {
    atMods.push(6144);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Jealous')) {
    const numBoosts = Math.max(0, defender.boosts.atk) + Math.max(0, defender.boosts.def) +
      Math.max(0, defender.boosts.spa) + Math.max(0, defender.boosts.spd) +
      Math.max(0, defender.boosts.spe);
    if (numBoosts > 1) {
      atMods.push(4096 + Math.floor(409.6 * numBoosts));
      desc.attackerAbility = attacker.ability;
      desc.foeBoosts = numBoosts;
    }
  }
  ...
  if (gen.num >= 9 && defender.hasAbility('Heatproof') && move.hasType('Fire')) {
    atMods.push(2048);
    desc.defenderAbility = defender.ability;
  } else if (defender.hasAbility('Brain of the Moon') &&
    (move.hasType('Psychic') || move.hasType('Dark'))) {
    atMods.push(2048);
    desc.defenderAbility = defender.ability;
  } else if (defender.hasAbility('Dark Border') &&
    (move.hasType('Fire') || move.hasType('Fairy'))) {
    atMods.push(2048);
    desc.defenderAbility = defender.ability;
  }
...
export function calculateDefenseTH(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const dfMods = calculateDfModsTH(
    gen,
    attacker,
    defender,
    move,
...
export function calculateDfModsTH(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const isSwordOfHisou = (attacker.hasAbility('Sword of Hisou') && field.hasWeather());
  if (isSwordOfHisou && hitsPhysical) {
    desc.attackerAbility = attacker.ability;
    dfMods.push(3072);
  }
...
function calculateBaseDamageTH(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  basePower: number,
  ...
  if (
    ...
    if (
      ...
      (field.hasWeather('Rain') && move.hasType('Fire') && !attacker.hasAbility('Hakkero'))
    ) {
...
export function calculateFinalModsTH(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (attacker.hasAbility('Neuroforce') && typeEffectiveness > 1) {
    ...
    finalMods.push(8192);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Black and White') && typeEffectiveness < 1) {
    finalMods.push(2048);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Black and White') && typeEffectiveness > 1) {
    finalMods.push(6144);
