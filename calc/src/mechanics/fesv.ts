...
export function calculateFESV(
  ...
  const defenderAbilityIgnored = defender.hasAbility(
    'Air Control', 'Airborne Armor', 'Armor Tail', 'Aroma Veil', 'Aura Break', 'Battle Armor',
  ...
  const type1Effectiveness = getMoveEffectiveness(
    ...
    false,
    false,
    attacker.hasAbility('Air Control')
  );
  ...
  if ((defender.hasAbility('Wonder Guard') && typeEffectiveness <= 1) ||
      ...
      (move.hasType('Ground') &&
        ...
        !defender.hasItem('Iron Ball') && defender.hasAbility('Levitate', 'Airborne Armor')) ||
      (move.flags.bullet && defender.hasAbility('Bulletproof')) ||
      (move.flags.sound && !move.named('Clangorous Soul') && defender.hasAbility('Soundproof')) ||
      (move.priority > 0 && defender.hasAbility('Queenly Majesty', 'Dazzling', 'Armor Tail')) ||
      (move.hasType('Ground') && defender.hasAbility('Earth Eater', 'Air Control')) ||
  ...
  const basePower = calculateBasePowerFESV(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackFESV(gen, attacker, defender, move, field, desc, isCritical);
  const attackStat =
  ...
  const defense = calculateDefenseFESV(gen, attacker, defender, move, field, desc, isCritical);
  ...
  const baseDamage = calculateBaseDamageFESV(
    gen,
    attacker,
    defender,
    basePower,
  ...
  const finalMods = calculateFinalModsFESV(
    gen,
    attacker,
    defender,
    move,
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateFESV(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (move.timesUsed! > 1 || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAttack = calculateAttackFESV(gen, attacker, defender, move,
        field, desc, isCritical);
      const newDefense = calculateDefenseFESV(gen, attacker, defender, move,
        field, desc, isCritical);
      ...
      const newBasePower = calculateBasePowerFESV(
        gen,
        attacker,
        defender,
        move,
      ...
      const newBaseDamage = calculateBaseDamageFESV(
        gen,
        attacker,
        defender,
        newBasePower,
      ...
      const newFinalMods = calculateFinalModsFESV(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerFESV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const bpMods = calculateBPModsFESV(
    gen,
    attacker,
    defender,
    move,
...
export function calculateBPModsFESV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if ((attacker.hasAbility('Technician') && basePower <= 60) ||
    ...
    (attacker.hasAbility('Sharpness', 'Ancient Marble') && move.flags.slicing)
  ) {
    bpMods.push(6144);
    desc.attackerAbility = attacker.ability;
  }
...
export function calculateAttackFESV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const atMods = calculateAtModsFESV(gen, attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsFESV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
export function calculateDefenseFESV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const dfMods = calculateDfModsFESV(
    gen,
    attacker,
    defender,
    move,
...
export function calculateDfModsFESV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
function calculateBaseDamageFESV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  basePower: number,
...
export function calculateFinalModsFESV(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (defender.hasAbility('Solid Rock', 'Filter', 'Prism Armor', 'Airborne Armor') &&
    typeEffectiveness > 1) {
    finalMods.push(3072);
    desc.defenderAbility = defender.ability;
  }
