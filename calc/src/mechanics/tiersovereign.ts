...
export function calculateTS(
  ...
  const basePower = calculateBasePowerTS(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackTS(gen, attacker, defender, move, field, desc, isCritical);
  const attackStat =
  ...
  const defense = calculateDefenseTS(gen, attacker, defender, move, field, desc, isCritical);
  ...
  const baseDamage = calculateBaseDamageTS(
    gen,
    attacker,
    defender,
    basePower,
  ...
  const finalMods = calculateFinalModsTS(
    gen,
    attacker,
    defender,
    move,
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateTS(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (move.timesUsed! > 1 || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAttack = calculateAttackTS(gen, attacker, defender, move,
        field, desc, isCritical);
      const newDefense = calculateDefenseTS(gen, attacker, defender, move,
        field, desc, isCritical);
      ...
      const newBasePower = calculateBasePowerTS(
        gen,
        attacker,
        defender,
        move,
      ...
      const newBaseDamage = calculateBaseDamageTS(
        gen,
        attacker,
        defender,
        newBasePower,
      ...
      const newFinalMods = calculateFinalModsTS(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerTS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const bpMods = calculateBPModsTS(
    gen,
    attacker,
    defender,
    move,
...
export function calculateBPModsTS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
export function calculateAttackTS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const atMods = calculateAtModsTS(gen, attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsTS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
export function calculateDefenseTS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const dfMods = calculateDfModsTS(
    gen,
    attacker,
    defender,
    move,
...
export function calculateDfModsTS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
function calculateBaseDamageTS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  basePower: number,
...
export function calculateFinalModsTS(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
