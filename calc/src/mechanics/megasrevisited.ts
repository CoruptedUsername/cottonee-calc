...
export function calculateMEGASR(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const ignoresNeutralizingGas = [
    'Multitype', 'Neutralizing Gas', 'Stance Change', 'Zen Mode',
  ];
  ...
  const isCritical =
    (move.isCrit || (attacker.hasAbility('Merciless') && defender.hasStatus())) &&
    !defender.hasAbility('Battle Armor', 'Shell Armor') && move.timesUsed === 1;
  ...
  if (!move.isZ && !noTypeChange) {
    ...
    if (isPixilate || isRefrigerate || isAerilate || isNormalize || isIntoxicate) {
      desc.attackerAbility = attacker.ability;
    }
    if (isPixilate || isRefrigerate || isAerilate || isIntoxicate) {
  ...
  if ((defender.hasAbility('Wonder Guard') && typeEffectiveness <= 1) ||
      ...
      (move.hasType('Bug') && defender.hasAbility('Bug Zapper')) ||
    (move.hasType('Ground') && defender.hasAbility('Earth Eater')) ||
      ...
      (move.priority > 0 && defender.hasAbility('Armor Tail'))
  ) {
    desc.defenderAbility = defender.ability;
    return result;
  }
  ...
  const basePower = calculateBasePowerMEGASR(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackMEGASR(gen, attacker, defender, move, field, desc, isCritical);
  const attackStat = attacker.hasAbility('Nostalgia Trip')
    ? move.hasType('Normal', 'Fighting', 'Flying', 'Poison', 'Ground', 'Rock', 'Bug', 'Ghost',
      'Steel') ? 'atk' : 'spa'
  ...
  const defense = calculateDefenseMEGASR(gen, attacker, defender, move, field, desc, isCritical);

  // #endregion
  // #region Damage

  const baseDamage = calculateBaseDamageMEGASR(
    gen,
    attacker,
    basePower,
    attack,
  ...
  const finalMods = calculateFinalModsMEGASR(
    gen,
    attacker,
    defender,
    move,
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateMEGASR(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (move.timesUsed! > 1 || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAtk = calculateAttackMEGASR(gen, attacker, defender, move, field, desc, isCritical);
      const newDef = calculateDefenseMEGASR(gen, attacker, defender, move, field, desc, isCritical);
      ...
      attacker.hasAbility('Aerilate', 'Galvanize', 'Intoxicate', 'Pixilate', 'Refrigerate');
      ...
      const newBasePower = calculateBasePowerMEGASR(
        gen,
        attacker,
        defender,
        move,
      ...
      const newFinalMods = calculateFinalModsMEGASR(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerMEGASR(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const bpMods = calculateBPModsMEGASR(
    gen,
    attacker,
    defender,
    move,
...
export function calculateBPModsMEGASR(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if ((attacker.hasAbility('Technician') && basePower <= 60) ||
    ...
    bpMods.push(4915);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Ice Scales') && move.hasType('Ice')) {
    bpMods.push(6144);
    desc.attackerAbility = attacker.ability;
  }
...
export function calculateAttackMEGASR(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const atMods = calculateAtModsMEGASR(attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsMEGASR(
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  field: Field,
  ...
  if ((attacker.hasAbility('Guts') && attacker.status && move.category === 'Physical') ||
    ...
    atMods.push(8192);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Brain Power') && move.category === 'Special') {
    atMods.push(8192);
    desc.attackerAbility = attacker.ability;
  }
...
export function calculateDefenseMEGASR(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  let defense: number;
  const defenseStat = move.overrideDefensiveStat || attacker.hasAbility('Nostalgia Trip')
    ? move.hasType('Normal', 'Fighting', 'Flying', 'Poison', 'Ground', 'Rock', 'Bug', 'Ghost',
      'Steel') ? 'def' : 'spd'
  ...
  const dfMods = calculateDfModsMEGASR(
    gen,
    defender,
    field,
    desc,
...
export function calculateDfModsMEGASR(
  gen: Generation,
  defender: Pokemon,
  field: Field,
  desc: RawDesc,
  ...
  } else if (defender.hasAbility('Exoskeleton') && hitsPhysical) {
    dfMods.push(8192);
    desc.defenderAbility = defender.ability;
...
function calculateBaseDamageMEGASR(
  gen: Generation,
  attacker: Pokemon,
  basePower: number,
  attack: number,
...
function calculateFinalModsMEGASR(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
