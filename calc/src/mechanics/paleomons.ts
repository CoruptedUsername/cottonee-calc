...
export function calculatePM(
  ...
  if ((defender.hasAbility('Wonder Guard') && typeEffectiveness <= 1) ||
    ...
    (move.hasType('Ground') &&
      ...
      !defender.hasItem('Iron Ball') && (defender.hasAbility('Levitate', 'Eelevate') ||
        (defender.hasAbility('First Flight') && defender.abilityOn))) ||
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculatePM(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
...
export function calculateBasePowerPM(
  ...
  case 'Payback':
    ...
    break;
  case 'Bolt Beak':
    basePower = move.bp * (turnOrder !== 'first' ? 1.5 : 1);
    desc.moveBP = basePower;
    break;
  case 'Fishious Rend':
  ...
  case 'Tail Sear':
    ...
    break;
  case 'Draconic Rend':
    basePower = move.bp * (switching ? 1.5 : 1);
...
export function calculateAttackPM(
  ...
  const attackStat =
    move.named('Body Press')
      ...
      : (move.category === 'Special' && !attacker.hasAbility('Head Barrage')
        ? 'spa' : 'atk');
  desc.attackEVs =
...
function calculateBaseDamagePM(
  ...
  if (
    ...
    if (
      ...
      isMegaSol ? desc.attackerAbility = attacker.ability : desc.weather = field.weather;
    } else if (field.hasWeather('Snow') && move.named('Bleakwind Storm')) {
      baseDamage = pokeRound(OF32(baseDamage * 6144) / 4096);
      desc.weather = field.weather;
    }
  }
