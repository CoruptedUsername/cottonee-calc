...
export function calculateFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (attacker.hasAbility('Neutral Match', 'Neutralizing Gas') &&
  ...
  if (defender.hasAbility('Neutral Match', 'Neutralizing Gas') &&
  ...
  const isGhostRevealed =
    attacker.hasAbility('Mind\'s Eye', 'Prideful', 'Scrappy') ||
  ...
  const basePower = calculateBasePowerFEVGC(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackFEVGC(gen, attacker, defender, move, field, desc, isCritical);
  // #endregion

  // #region (Special) Defense

  const defense = calculateDefenseFEVGC(gen, attacker, defender, move, field, desc, isCritical);
  ...
  const baseDamage = calculateBaseDamageFEVGC(
    gen,
    attacker,
    defender,
    basePower,
  ...
  const applyBurn =
    ...
    move.category === 'Physical' &&
    !attacker.hasAbility('Careless', 'Guts', 'Migrate', 'Unsettling') &&
    !move.named('Facade');
  desc.isBurned = applyBurn;
  const finalMods = calculateFinalModsFEVGC(
    gen,
    attacker,
    defender,
    move,
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateFEVGC(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (move.timesUsed! > 1 || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAttack = calculateAttackFEVGC(gen, attacker, defender, move,
        field, desc, isCritical);
      const newDefense = calculateDefenseFEVGC(gen, attacker, defender, move,
        field, desc, isCritical);
      ...
      const newBasePower = calculateBasePowerFEVGC(
        gen,
        attacker,
        defender,
        move,
      ...
      const newBaseDamage = calculateBaseDamageFEVGC(
        gen,
        attacker,
        defender,
        newBasePower,
      ...
      const newFinalMods = calculateFinalModsFEVGC(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const bpMods = calculateBPModsFEVGC(
    gen,
    attacker,
    defender,
    move,
...
export function calculateBPModsFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (attacker.hasAbility('Dusty Bugs') && (move.secondaries || move.named('Order Up')) &&
    !move.isMax && attacker.curHP() <= attacker.maxHP() / 3) {
    bpMods.push(6144);
    desc.attackerAbility = attacker.ability;
  }
  ...
  if ((attacker.hasAbility('Blazing Passion', 'Careless', 'Down In Flames', 'Reckless',
    'Underestimate') &&
...
export function calculateAttackFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const atMods = calculateAtModsFEVGC(gen, attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if ((attacker.hasAbility('Slow Start') && attacker.abilityOn &&
    ...
    (attacker.hasAbility('Water Bubble', 'Bubble Burster') && move.hasType('Water')) ||
...
export function calculateDefenseFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const dfMods = calculateDfModsFEVGC(
    gen,
    attacker,
    defender,
    move,
...
export function calculateDfModsFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
function calculateBaseDamageFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  basePower: number,
...
export function calculateFinalModsFEVGC(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (defender.hasAbility('Fluffy', 'Grim Plumage') && move.flags.contact &&
    ...
    (defender.hasAbility('Ice Scales', 'Vital Scales') && move.category === 'Special')
  ) {
    finalMods.push(2048);
    desc.defenderAbility = defender.ability;
  }

  if (defender.hasAbility('Lithoproof', 'Solid Rock', 'Filter', 'Prism Armor', 'Unfiltered') &&
    typeEffectiveness > 1) {
    finalMods.push(3072);
    desc.defenderAbility = defender.ability;
  }
