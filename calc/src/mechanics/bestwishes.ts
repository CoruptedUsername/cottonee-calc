...
import {
  ...
  isGrounded,
  OF16, OF32,
  pokeRound, getShellSideArmCategory,
} from './util';

export function calculateBWFYB(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if ((attacker.hasAbility('Mold Breaker', 'Teravolt', 'Turboblaze') ||
    (attacker.hasItem('Dragon Gem') && move.hasType('Dragon'))) && defenderAbilityIgnored) {
  ...
  if (move.named('Weather Ball')) {
    ...
    move.type = 'Ground';
  } else if (move.named('Brick Break') || (move.hasType('Fighting') &&
    attacker.hasItem('Fighting Gem'))) {
  ...
  const basePower = calculateBasePowerBWFYB(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackBWFYB(gen, attacker, defender, move, field, desc, isCritical);
  ...
  const defense = calculateDefenseBWFYB(gen, attacker, defender, move, field, desc, isCritical);

  // #endregion
  // #region Damage

  const baseDamage = calculateBaseDamageBWFYB(
    gen,
    attacker,
    basePower,
    attack,
  ...
  const finalMods = calculateFinalModsBWFYB(
    gen,
    attacker,
    defender,
    move,
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateBWFYB(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (move.timesUsed! > 1 || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAtk = calculateAttackBWFYB(gen, attacker, defender, move, field, desc, isCritical);
      const newDef = calculateDefenseBWFYB(gen, attacker, defender, move, field, desc, isCritical);
      ...
      const newBasePower = calculateBasePowerBWFYB(
        gen,
        attacker,
        defender,
        move,
      ...
      const newFinalMods = calculateFinalModsBWFYB(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerBWFYB(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const bpMods = calculateBPModsBWFYB(
    gen,
    attacker,
    defender,
    move,
...
export function calculateBPModsBWFYB(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (move.named('Solar Blade') && field.hasWeather() && !field.hasWeather('Sun')) {
    bpMods.push(3072);
    desc.weather = field.weather;
  }
  ...
  if ((move.named('Facade') && attacker.hasStatus('brn', 'par', 'psn', 'tox', 'frz', 'slp')) ||
...
export function calculateAttackBWFYB(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const atMods = calculateAtModsBWFYB(attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsBWFYB(
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  field: Field,
  ...
  if ((attacker.hasItem('Thick Club') &&
      ...
      (attacker.hasItem('Light Ball') && (attacker.name.startsWith('Pikachu') ||
        attacker.name.startsWith('Emolga')) && !move.isZ)
  ) {
    atMods.push(8192);
    desc.attackerItem = attacker.item;
  } else if (
...
export function calculateDefenseBWFYB(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const dfMods = calculateDfModsBWFYB(
    gen,
    defender,
    field,
    desc,
...
export function calculateDfModsBWFYB(
  gen: Generation,
  defender: Pokemon,
  field: Field,
  desc: RawDesc,
  ...
  } else if (hitsPhysical && defender.hasAbility('Fur Coat')) {
    dfMods.push(8192);
    desc.defenderAbility = defender.ability;
  }
  ...
  if (defender.hasStatus('frz')) {
    dfMods.push(2048);
    desc.isBestWishesFrozen = true;
  }

  if (defender.hasType('Ice') && field.hasWeather('Snow') && hitsPhysical) {
    dfMods.push(6144);
    desc.weather = field.weather;
  }
  return dfMods;
...
function calculateBaseDamageBWFYB(
  gen: Generation,
  attacker: Pokemon,
  basePower: number,
  attack: number,
...
function calculateFinalModsBWFYB(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (field.defenderSide.isReflect && move.category === 'Physical' && !isCritical) {
    finalMods.push(field.gameType !== 'Singles' ? 2703 : 2048);
    desc.isReflect = true;
